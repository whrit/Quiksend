...
export const getSequenceFunnel = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return withAnalyticsTiming("getSequenceFunnel", organizationId, () =>
...
export const getSequenceStepRates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getSequenceStepRates", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
...
export const getSequenceABCompare = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getSequenceABCompare", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        const rows = await tx
          .select({
...
export const getMailboxVolume = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getMailboxVolume", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        const from = new Date(data.from);
        const to = new Date(data.to);

        const rows = await tx.execute<{ hour: string; sent: number; bounced: number }>(sql`
        SELECT
...
export const getWorkspaceOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getWorkspaceOverview", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        ...
        const dailyTrend = await tx.execute<{ day: string; replies: number; sent: number }>(sql`
      SELECT
...
export const getSequencePerformance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getSequencePerformance", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
...
export const getSequenceEventTimeline = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getSequenceEventTimeline", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        return tx.execute<{
          id: string;
          type: string;
...
export const getProspectWritebackLogs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getProspectWritebackLogs", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        const enrollments = await tx.query.enrollment.findMany({
          where: and(
            eq(tables.enrollment.organizationId, organizationId),
        ...
        const rows = await tx.query.crmWritebackLog.findMany({
          where: and(
...
export const getMailboxHealthSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { organizationId } = context.orgContext;
    return withAnalyticsTiming("getMailboxHealthSummary", organizationId, () =>
      withTenantTransaction(organizationId, async (tx) => {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
