...
function mockSelectChain(rows: unknown[]): void {
  vi.mocked(db.select).mockReturnValueOnce({
    from: vi.fn<any>().mockReturnValueOnce({
      where: vi.fn<any>().mockReturnValueOnce({
        limit: vi.fn<any>().mockResolvedValueOnce(rows),
      }),
    }),
  } as never);
}
...
describe("Health Reconciliation Handler", () => {
  ...
  it("bounds scan to 100 records per state type", async () => {
    const limitMock = vi.fn<any>().mockResolvedValueOnce([]);
  ...
  it("logs completion with elapsed time and counts", async () => {
    ...
    const completionLog = logCalls.find(
      (call) => typeof call[1] === "string" && call[1].includes("complete"),
    );
